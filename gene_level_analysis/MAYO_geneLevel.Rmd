...
#Added to track all sig Genes - JKG
ADCntrl = list( Up = list( CBE = CBE_UP_AD_Cntrl, TCX = TCX_UP_AD_Cntrl), 
                Down = list( CBE = CBE_Down_AD_Cntrl, TCX = TCX_Down_AD_Cntrl)
              )
...
#Added to track all sig Genes - JKG
foo <- DE[ DE$Comparison == 'APOE42-APOE40', ]
table(foo$Tissue.ag, foo$Direction)
CBE_UP_APOE <- foo[ foo$Tissue.ref == 'CBE' & foo$Direction == 'UP', ]$ensembl_gene_id
CBE_Down_APOE <- foo[ foo$Tissue.ref == 'CBE' & foo$Direction == 'DOWN', ]$ensembl_gene_id
TCX_UP_AD_APOE <- foo[ foo$Tissue.ref == 'TCX' & foo$Direction == 'UP', ]$ensembl_gene_id
TCX_Down_APOE <- foo[ foo$Tissue.ref == 'TCX' & foo$Direction == 'DOWN', ]$ensembl_gene_id


APOE42_40 = list( Up = list( CBE = CBE_UP_APOE, TCX = TCX_UP_AD_APOE), 
                Down = list( CBE = CBE_Down_APOE, TCX = TCX_Down_APOE)
              )
...
#Added to track all sig Genes - JKG
foo <- DE[ DE$Comparison == 'AD-CONTROL', ]
table(foo$Tissue.ag, foo$Direction)
CBE_UP_Male <- foo[ foo$Tissue.ref == 'CBE' & foo$Sex.ref =='MALE' & foo$Direction == 'UP', ]$ensembl_gene_id
CBE_Down_Male <- foo[ foo$Tissue.ref == 'CBE' & foo$Sex.ref =='MALE' & foo$Direction == 'DOWN', ]$ensembl_gene_id
CBE_UP_Female <- foo[ foo$Tissue.ref == 'CBE' & foo$Sex.ref =='FEMALE' & foo$Direction == 'UP', ]$ensembl_gene_id
CBE_Down_Female <- foo[ foo$Tissue.ref == 'CBE' & foo$Sex.ref =='FEMALE' & foo$Direction == 'DOWN', ]$ensembl_gene_id

TCX_UP_Male <- foo[ foo$Tissue.ref == 'TCX' & foo$Sex.ref =='MALE' & foo$Direction == 'UP', ]$ensembl_gene_id
TCX_Down_Male <- foo[ foo$Tissue.ref == 'TCX' & foo$Sex.ref =='MALE' & foo$Direction == 'DOWN', ]$ensembl_gene_id
TCX_UP_Female <- foo[ foo$Tissue.ref == 'TCX' & foo$Sex.ref =='FEMALE' & foo$Direction == 'UP', ]$ensembl_gene_id
TCX_Down_Female <- foo[ foo$Tissue.ref == 'TCX' & foo$Sex.ref =='FEMALE' & foo$Direction == 'DOWN', ]$ensembl_gene_id

Sex = list( Up = list( Female = list( CBE = CBE_UP_Female, TCX = TCX_UP_Female), Male = list( CBE = CBE_UP_Male, TCX = TCX_Down_Male)), 
                Down = list( Female = list( CBE = CBE_Down_Female, TCX = TCX_Down_Female), Male = list( CBE = CBE_Down_Male, TCX = TCX_Down_Male))
          )
Comp = list( ADCntrl = ADCntrl, APOE42_40 = APOE42_40, Sex = Sex )
```
...
### Store files in /output directory
```{r dir.store, include=FALSE, eval=TRUE, cache=FALSE}
# Store covariates
COVARIATES = rownameToFirstColumn(COVARIATES, 'SampleID')
write.table(COVARIATES, file = '../output/MAYO/MAYO_CBE_TCX_Covariates.tsv', sep = '\t', row.names=F, quote=F)

# Store filtered counts
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_Counts.tsv', sep = '\t', row.names=F, quote=F)

# Store filtered counts
NEW.COUNTS %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_eCounts.tsv', sep = '\t', row.names=F, quote=F)

# Store logCPM
CQN.GENE_EXPRESSION$y %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_logCPM.tsv', sep = '\t', row.names=F, quote=F)

# Store cqn offsets
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_offset.tsv', sep = '\t', row.names=F, quote=F)

# Store design matrix
MODEL1 %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_Design.tsv', sep = '\t', row.names=F, quote=F)

# Store surrogate variables
SURR.VAR %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_SurVar.tsv', sep = '\t', row.names=F, quote=F)

# Store residual gene expression for eQTL analysis
RESIDUAL.SVA.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_eQTLResidualExpression.tsv', sep = '\t', row.names=F, quote=F)

# Store residual gene expression for network analysis
RESIDUAL.NET.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)

# Store differential expression results
rbindlist(all.diff.exp, use.names = T, fill = T, idcol = 'Model') %>%
  write.table(file = '../output/MAYO/MAYO_CBE_TCX_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)

# Store all model fits
save(list = 'all.fit', file = '../output/MAYO/MAYO_CBE_TCX_allfits.RData')

save( file = '../output/MAYO/MAYO_DiffExpression_ENSGs.RDat')
file <- File(path = '../output/MAYO/MAYO_DiffExpression_ENSGs.RDat', parent = 'syn8466812')
file <- synStore(file)
