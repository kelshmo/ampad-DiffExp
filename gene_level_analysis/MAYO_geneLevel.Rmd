...
COUNT_TC <- synGet(COUNT_TC_ID)$path %>%
...
COUNT_CE <- synGet(COUNT_CE_ID)$path %>%
  read.table(header=T, sep='\t', check.names = F, row.names = 1)
...
METADATA_TC <- synGet(METADATA_TC_ID)$path %>%
...
METADATA_PICARD_CE <- synGet(METADATA_PICARD_CE_ID)$path %>%
  read.table(sep='\t', header=T, stringsAsFactors=F) %>% 
...
METADATA_PICARD_TC <- synGet(METADATA_PICARD_TC_ID)$path %>%
  read.table(sep='\t', header=T, stringsAsFactors=F) %>% 
  dplyr::rename(ID = sample)
...
SAMPLES.EXCLUDE = c(read.table(synGet('syn6126119', version = 3)$path, sep = '\t', header = T, stringsAsFactors = F)$`Sample Name`,
                    synGet('syn6126114', version = 3)$`Sample Name`)
...
CONTROL.PROBES = fread(synGet(CONTROL.PROBES_ID)$path, data.table = F) %>%
  dplyr::mutate(ID = x) %>%
  tidyr::separate(ID, c('ensembl_gene_id', 'position'), sep = '\\.')
...
knit2synapse::knitToFolderEntity(file = "./MAYO_geneLevel.Rmd",
                                 parentId = "syn8672415",
